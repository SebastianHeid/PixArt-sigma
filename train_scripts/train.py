    ...
    for epoch in range(start_epoch + 1, config.num_epochs + 1):
        ...
        for step, batch in enumerate(tqdm(train_dataloader)):
            ...
            if (step + 1) % config.log_interval == 0 or (step + 1) == 1:
                ...
                info = (
                    f"Step/Epoch [{global_step}/{epoch}][{step + 1}/{len(train_dataloader)}]:total_eta: {eta}, "
...
if __name__ == "__main__":
    ...
    validation_noise = (
        torch.randn(1, 4, latent_size, latent_size, device="cpu")
        if getattr(config, "deterministic_validation", False)
        else None
    )
    ...
    if config.visualize:
        ...
        if accelerator.is_main_process and not skip:
            if config.data.load_t5_feat and (tokenizer is None or text_encoder is None):
                logger.info(
                    f"Loading text encoder and tokenizer from {args.pipeline_load_from} ..."
                )
                tokenizer = T5Tokenizer.from_pretrained(
                    args.pipeline_load_from, subfolder="tokenizer"
                )
